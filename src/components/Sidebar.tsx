...
import { Avatar, Collapse, Container, CssBaseline, MenuItem, styled, SvgIcon, Toolbar } from '@mui/material';
...
export default function SidebarDrawer({ children }) {
  ...
  const isInChildRoute = (menu) => {
    const isIn = menu?.children?.length && (menu?.children as any[]).some(({ route }) => location.pathname.includes(route));
    return isIn;
  };

  const menuContent = (
    <>
      <List sx={{ width: '100%', flex: 1, bgcolor: 'background.primary' }} component="nav">
          ...
          return (
            <Fragment key={`menu-item-${menu.title}-${index}`}>
              ...
              >
                ...
                <Typography sx={{ color: 'white' }} component="div" fontSize={pxToRem(20)}>
                  {menu.title}
                </Typography>
              </AutMenuItem>
              {menu.children && (
